    ...
    I::Item: Clone,
{
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        if let Some(iter) = iter {
            ...
            // Mutate underlying base to calculate corresponding hints.
            // But don't forget original value.
            let safe_base = indices.base;
            let a = {
                indices.base = a; // HERE: find a better way out.
                indices.size_hint().0
            };
            let b = b
                .map(|b| {
