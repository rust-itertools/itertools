...
pub struct MultizipFallback<T, X> {
    t: T,
    def: X,
}
...
///
/// The iterator `MultizipFallback<(A, C, ..., M), (B, D, ..., N)>` is formed from a tuple of tuples where
...
/// element types of each subiterator.
///
/// If extending with fallback values isn't needed, use [`izip!()`].
///
...
/// ```
pub fn multizip_fallback<T, U, X>(t: U) -> MultizipFallback<T, X>
where
    MultizipFallback<T, X>: From<U>,
    MultizipFallback<T, X>: Iterator,
{
    MultizipFallback::from(t)
}
...
macro_rules! impl_zip_all_iter {
    ($(($B:ident, $C:ident)),*) => (
        ...
        From<($(($B,$C),)*)> for MultizipFallback<($($B::IntoIter,)*), ($($C,)*)> {
            fn from(t: ($(($B,$C),)*)) -> Self {
                let ($(($B,$C),)*)= t;
                MultizipFallback {
                    t: ($($B.into_iter(),)*),
        ...
        impl<$($B,$C),*> Iterator for MultizipFallback<($($B,)*), ($($C,)*)>
            where
            $(
                $B: Iterator<Item = $C>,
                $C: Clone,
        ...
        #[allow(non_snake_case)]
        impl<$($B,$C),*> ExactSizeIterator for MultizipFallback<($($B,)*), ($($C,)*)>
        where
        $(
